...
fig = go.Figure(go.Scattermapbox(
    lat=common_df1['Latitude'],
    lon=common_df1['Longitude'],
    mode='markers',
    marker=dict(
        size=10,
        color=[color_map.get(level, 'gray') for level in common_df1['price_level']],
        opacity=0.7
    ),
    text=common_df1.apply(lambda row: f"""
    <b>지역:</b> {row['Neighborhood']}<br>
    <b>매매가:</b> ${row['SalePrice']:,}<br>
    <b>지상면적:</b> {row['GrLivArea']} sqft<br>
    <b>마감 품질:</b> {row['OverallQual']}<br>
...
fig.update_layout(
    mapbox_style="open-street-map",
    mapbox_center={"lat": common_df1["Latitude"].mean(), "lon": common_df1["Longitude"].mean()},
    mapbox_zoom=12,
