...
# 각 그룹별 지역 목록 추출
high_neighborhoods = df[df['price_level'] == 'High']['Neighborhood'].unique()
mid_neighborhoods = df[df['price_level'] == 'Mid']['Neighborhood'].unique()
low_neighborhoods = df[df['price_level'] == 'Low']['Neighborhood'].unique()

# 각 그룹별 지역을 데이터프레임으로 생성하고, 가격 레벨을 추가
high_df_neighborhoods = pd.DataFrame(high_neighborhoods, columns=['Neighborhood'])
high_df_neighborhoods['Price_Level'] = 'High'

mid_df_neighborhoods = pd.DataFrame(mid_neighborhoods, columns=['Neighborhood'])
mid_df_neighborhoods['Price_Level'] = 'Mid'

low_df_neighborhoods = pd.DataFrame(low_neighborhoods, columns=['Neighborhood'])
low_df_neighborhoods['Price_Level'] = 'Low'

# 세 개의 데이터프레임을 합침
neighborhoods_df = pd.concat([high_df_neighborhoods, mid_df_neighborhoods, low_df_neighborhoods], ignore_index=True)
